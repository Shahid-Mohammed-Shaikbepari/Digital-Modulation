...
% plot(1:length(Tx_vec1), Tx_vec1); hold on %plot the signal which will be transmitted.

Tx_vec2 = ifft(data_16QAM_sequential,'symmetric'); %to accumulate all the symbols going to be transmitted, convert them to discrete format.
%plot(1:length(Tx_vec2), Tx_vec2);
%legend('Gray', 'Sequential');

%% Noise addition

SNR_total =  Eb_No(i) + 10*log10(bitsPerSymbol);
...
% sFigure1 = scatterplot(add_noiseToSequential,1,0,'r.'); hold on
% sFigure2 = scatterplot(data_16QAM_sequential,1,0, 'k*',sFigure1);
...
BER(i) = ber_gray;
BER1(i) = ber_sequential;
%SER
serError1 = 0;
serError2 = 0;
for ii = 1:length(data_bitsPerSymbol)
    if gray_toBinary(ii) ~=  data_bitsPerSymbol(ii)
        serError1 = serError1 + 1;
    end
     if sequential_toBinary(ii) ~=  data_bitsPerSymbol(ii)
...
SER(i) = ser_gray;
Packet_loss(i) = serError1;
evm = lteEVM(add_noiseToGray, data_16QAM_gray);
EVM(i) = evm.Peak;
end
%% Plots
figure;
semilogy(Eb_No, BER, 'linewidth', 1.5);
title('BER v/s Eb/No for 64QAM');
xlabel('Eb/No (dB)');
ylabel('BER');

figure;
semilogy(Eb_No, SER,'linewidth', 1.5);
title('SER v/s Eb/No for 64QAM');
xlabel('Eb/No (dB)');
ylabel('SER');

figure;
semilogy(Eb_No, EVM,'linewidth', 1.5);
title('EVM v/s Eb/No for 64QAM');
xlabel('Eb/No (dB)');
ylabel('EVM');


figure;
semilogy(Eb_No, Packet_loss,'linewidth', 1.5);
title('Packet loss v/s Eb/No for 64QAM');
xlabel('Eb/No (dB)');
ylabel('Packet loss');
